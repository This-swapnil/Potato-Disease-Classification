...
def main(config_path):
    ...
    logging.info("Layers defining...")

    LAYERS = [
        tf.keras.layers.Input(shape = tuple(params["img_size"])),
        ...
        tf.keras.layers.Dense(params['class_no'], activation = "softmax")
    ]
    ...
    path_to_model_dir = os.path.join(config['source_data']['local_dir'], config['source_data']['model_dir'])
    create_directories([path_to_model_dir])

    path_to_model = os.path.join(path_to_model_dir, config['source_data']['init_model_file'])
